...
class _AddPersonScreenState extends ConsumerState<AddPersonScreen> {
  ...
  Future<void> _submit() async {
    ...
    final person = Person()
      ...
      ..avatar = '👤' // Still stored but UI uses IconUtils
  ...
  Widget build(BuildContext context) {
    final color = _relationColor(_selectedType);
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          if (_isLoading)
            ...
            TextButton(onPressed: _submit, child: const Text('Ajouter', style: TextStyle(fontWeight: FontWeight.w700))),
        ],
      ),
      body: Form(
        key: _formKey,
          ...
          children: [
            ...
            Center(
              child: Container(
                ...
                child: Center(child: Icon(IconUtils.getIconForRelation(_selectedType.name), size: 42, color: color)),
              ),
            ).animate().scale(duration: 400.ms, curve: Curves.elasticOut),
            const SizedBox(height: 24),
            
            _buildSection(context, title: 'Nom', child: TextFormField(
              controller: _nameController,
              decoration: const InputDecoration(hintText: 'Prénom et/ou nom', prefixIcon: Icon(Icons.person_outline_rounded)),
              validator: (v) => v == null || v.trim().isEmpty ? 'Le nom est requis' : null,
              textCapitalization: TextCapitalization.words,
            )).animate().fadeIn(duration: 300.ms, delay: 50.ms),
            
            const SizedBox(height: 20),
            _buildSection(context, title: 'Type de relation', child: _buildRelationTypeGrid()).animate().fadeIn(duration: 300.ms, delay: 100.ms),
            
            const SizedBox(height: 20),
            _buildSection(context, title: 'Date de rencontre', child: _buildDatePicker(
              label: 'Quand vous êtes-vous rencontrés ?', value: _metAt, icon: Icons.handshake_outlined,
              onPicked: (d) => setState(() => _metAt = d),
            )).animate().fadeIn(duration: 300.ms, delay: 150.ms),
            
            const SizedBox(height: 20),
            _buildSection(context, title: 'Anniversaire', child: _buildDatePicker(
              label: 'Date de naissance', value: _birthday, icon: Icons.cake_outlined,
              onPicked: (d) => setState(() => _birthday = d),
            )).animate().fadeIn(duration: 300.ms, delay: 200.ms),
            
            const SizedBox(height: 20),
            _buildSection(context, title: 'Notes', child: TextFormField(
              controller: _notesController,
              maxLines: 4,
              decoration: const InputDecoration(hintText: 'Quelque chose à retenir...', alignLabelWithHint: true),
      ...
      children: RelationType.values.map((type) {
        ...
        return GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              color: selected ? color.withValues(alpha: 0.1) : AppColors.surface2,
            ...
            child: Row(
              ...
              children: [
                Icon(IconUtils.getIconForRelation(type.name), size: 16, color: selected ? color : AppColors.textSecondary),
                const SizedBox(width: 8),
                Text(
                  _typeLabel(type),
                  style: TextStyle(fontSize: 13, fontWeight: selected ? FontWeight.w700 : FontWeight.w400, color: selected ? color : AppColors.textSecondary),
                ),
              ],
            ),
          ),
  ...
  Widget _buildDatePicker({required String label, required DateTime? value, required IconData icon, required ValueChanged<DateTime> onPicked}) {
    return GestureDetector(
      onTap: () async {
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          Text(value != null ? DateFormat('dd MMMM yyyy', 'fr_FR').format(value) : label,
            style: TextStyle(fontSize: 14, color: value != null ? AppColors.textPrimary : AppColors.textMuted),
          ),
          const Spacer(),
          if (value != null) const Icon(Icons.check_circle_rounded, size: 16, color: AppColors.accent),
        ]),
      ),
    );
  }
  ...
  Color _relationColor(RelationType t) {
    switch (t) {
